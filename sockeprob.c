...
{
    printk("assaf - socket connected, RDI=%lu, RSI=%lu, RDX=%lu\n", regs->di, regs->si, regs->dx);
    struct sockaddr* sa = (struct sockaddr*)regs->si;
    printk("assaf - sa_family is %u\n", sa->sa_family);
    /*if (sa->sa_family == 2)
    {
        printk("sa_data is %s\n", sa->sa_data);
    } */
...
{
    printk("assaf - socket connect finished\n");
}
int minit(void)
{
    printk("assaf - Module inserted\n");
